...
if (typeof window.AppointmentsManager === 'undefined') {
    class AppointmentsManager {
        constructor() {
            this.baseUrl = 'http://localhost:5000/api'; // Ensure consistent base URL
            this.init();
        }
        ...
        async cancelAppointment(appointmentId) {
            try {
                ...
                const response = await fetch(`${this.baseUrl}/bookings/cancel/${appointmentId}`, {
                    method: 'PATCH', // Changed to PATCH since we're updating status
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    },
                ...
                this.showError(error.message || 'Failed to cancel appointment');
            }
        }
    ...
    // Make AppointmentsManager available globally
    window.AppointmentsManager = AppointmentsManager;
}

// Initialize appointments manager only if it doesn't exist
